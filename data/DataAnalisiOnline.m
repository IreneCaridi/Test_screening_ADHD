...
dim=12;
Dati=readcell("NaN.csv");
[Dati,PosDatix,PosDatiy,ResponseTimes,Responses,Scores,Sizes,isDifficult]=leggiOnline("NaN.csv",dim);
Totptx=sum(Scores); %punti totali
FrameRate=Dati{2,47}; %frequenza dei fotogrammi
TimeOut=0;
TimeIn=0;
for i=1:dim
...
for i=1:dim
    ...
    for j=1:1 %ricavo delta movimento
...
Generali=readtable("DataCollection.csv", VariableNamingRule="preserve");

addpath("C:\Users\irene.LAPTOP-IPO788K1\Desktop\Test_Validazione\data");

M=[Dati{2,41},TotScore,ScoreDiff,ScoreEasy,TimeAverage,TimeAverageDiff,TimeAverageEasy,PCOutTot,PCOutDiff,PCOutEasy,RoamTot,RoamDiff,RoamEasy,AverageDiff,progd,proge,DomandeIncomprese];
dlmwrite("DataCollection.csv",M,'delimiter',',','-append');
